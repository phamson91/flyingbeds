...
import TableWithCollap from './TableWithCollap';
...
import ModalDisplayPnr from './ModalDisplayPnr';
import { createClientComponentClient } from '@supabase/auth-helpers-nextjs';
import ModalVoidTicket from './ModalVoidTicket';

interface BookingsPageProps {
	expirationTime: number;
}
const ManageBookingsPageContent: React.FC<BookingsPageProps> = ({
	expirationTime,
}) => {
	...
	useEffect(() => {
		...
		const channel = supabase
			.channel('realtime manage bookings')
			.on(
				'postgres_changes',
				{
					event: '*',
		...
		<section>
			...
			{booking &&
				(modal === ETypeTicket.REFUNDING ||
					...
					<ModalRequest booking={booking} typeModal={modal} onClose={onClose} />
				)}
			{booking && modal === ETypeTicket.CANCEL && (
				<ModalCancel booking={booking} typeModal={modal} onClose={onClose} />
			)}
